        ...
        <el-form :model="ruleForm"
          ...
          <el-form-item label="账号"
            ...
            <el-input v-model.number="ruleForm.age"
                      placeholder="请输入账号或手机号"></el-input>
          </el-form-item>
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          postRequst('/storelogin', data).then((res) => {
            const sjuser = JSON.stringify(res.data.sjUser)
            if (res.code === 200) {
              window.sessionStorage.setItem('sjtoken', res.data.sjToken)
              window.sessionStorage.setItem('sj_user', sjuser)
              setTimeout(() => {
