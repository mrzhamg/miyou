  ...
  <div class="car-box">
    ...
    <div>
      <div class="car-top">
        ...
        <div class="add-select">
          ...
          <el-button type="primary" style="margin:0 10px;" size="small" icon="el-icon-plus" circle @click="dialogFormVisible1 = true"></el-button>
        </div>
      </div>
      <table cellpadding="5"
             cellspacing="5"
        ...
        <tr v-for="(item,index) in cars"
          ...
          <td>
            <router-link :to="'goodes?sid='+item.gid">
            <div class="sp-box">
              <div>
    ...
    <!-- 添加地址 -->
          <el-dialog title="收货地址" :visible.sync="dialogFormVisible1">
            <el-form :model="form1" :rules="form1Refs" ref="form1">
...
export default {
  data () {
    return {
      ...
      form1Refs: {
        ...
        phone: [{ required: true, message: '请输入收件人电话', max: 11, trigger: 'blur' }],
  ...
  methods: {
    ...
    submitForm (formName) {
      const address = { uid: this.$store.state.user.uid, aid: '', shoujianName: this.form1.name, sjrPhone: this.form1.phone, addres: this.form1.address }
      this.$refs[formName].validate((valid) => {
        if (valid) {
          console.log(this.form1)
          postRequst('/insertAddress', address).then((res) => {
            if (res.code === 200) {
              setTimeout(() => {
                location.reload()
              }, 200)
