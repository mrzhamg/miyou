    ...
    <div class="about-jbxx">
      ...
      <el-descriptions title=""
        ...
        <el-descriptions-item label="用户头像">
          <div class="img-box">
            <div class="userImg"><img :src="`http://localhost:8082`+user.imgurl"
                   alt=""></div>
        ...
        <el-descriptions-item label="注册手机号"> {{user.phone}}</el-descriptions-item>
      ...
      <template>
        <el-table :data="user.address"
          ...
          <el-table-column prop="shoujianName"
                           label="收件人"
                           width="180">
          </el-table-column>
          <el-table-column prop="sjrPhone"
                           label="收件人电话">
          </el-table-column>
          <el-table-column prop="addres"
                           label="地址">
          </el-table-column>
        ...
        <p class="addBtn"
           v-show="!user.address">
          <el-button @click="handleClick(address)"
                     type="primary"
                     size="small">添加地址</el-button>
        </p>
      </template>
      ...
      <el-form :model="addressForm"
        ...
        <el-form-item label="收件人"
          ...
          <el-input type="text"
                    class=""
                    placeholder="最多6个字符"
                    v-model="addressForm.shoujianren" />
        </el-form-item>
        ...
        <el-form-item class="form-btn">
          <el-button type="primary"
                     @click="editFormBtn('addressForm')"> 保存修改</el-button>
          <el-button type="primary"
...
export default {
  ...
  methods: {
    ...
    async getAddress (uid) {
      ...
      await getRequst('/getAddress', { uid: uid }).then((res) => {
        // console.log('resAddress=>', res.data.address)
...
.about {
  ...
  .about-jbxx {
    ...
    .img-box {
      ...
      .userImg {
        width: ((40 * @2rem) / @base-size);
        height: ((40 * @2rem) / @base-size);
        border-radius: 50%;
        margin-right: ((10 * @2rem) / @base-size);
    ...
    .aboutBtn {
      margin: ((20 * @2rem) / @base-size);
      text-align: center;
    }
    .address {
      margin: 0 ((5 * @2rem) / @base-size);
    }
    .addBtn {
      text-align: center;
    }
